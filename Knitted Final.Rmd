...
We will be working with data disclosed in 2008 - at the height of a major financial crisis primarily caused by an overabundance of risky home loans. While I won't be addressing the crisis in my analysis, its presence may be seen in <>
...
Now that our data is loaded, we can begin to manipulate it. 45 variables is a lot to work with - let's see if we can remove <>

If the percentage <>, we can safely discard the entities.
```{r first_looks}
# percent of primary applicants listed as more than one race
nrow(hmda_smpl %>% filter(!is.na(applicant_race_2)))/nrow(hmda_smpl) * 100

# pct interracial couple applicants
nrow(hmda_smpl %>% filter(applicant_race_1 != co_applicant_race_2))/nrow(hmda_smpl) * 100

# pct sex not recorded
nrow(hmda_smpl %>% filter(!(applicant_sex == 1 | applicant_sex == 2)))/nrow(hmda_smpl) * 100

# same-sex co-applicant
nrow(hmda_smpl %>% filter(applicant_sex == co_applicant_sex))/nrow(hmda_smpl) * 100
```
